...
class loginHelper {
	...
	function __construct($apps){
		...
		$this->schema = "heineken";
	}
	
	function doLogin($fbData){
		//pr($fbData);exit;
		if($fbData['status']==1 && $this->login==false){
			$this->apps->session->setSession($this->config['SESSION_NAME'],"WEB",$fbData['return']);
		}
	}
	...
	function goLogin($web=false){
		...
		$sql = "SELECT sm.* FROM social_member sm WHERE sm.username=\"{$username}\"  LIMIT 1";
	...
	*/
	...
	function setdatasessionuser($rs=false){
		...
		$campaign_id = $this->getCampaignID($id);

		$this->reset_try_login($rs);
		if($campaign_id)	{				
			$rs = array_merge($rs,$campaign_id);
		}if($pagestat)	{				
	...
	function getCampaignID($client_id=null){

		if($client_id==null) return false;
		$sql = "SELECT GROUP_CONCAT(id) as campaign_ids 
