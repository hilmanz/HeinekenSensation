...
class uploadHelper {
	...
	function resizeThisImage($image){
		...
		$path = $CONFIG['PATH'];
		//large
		$ori_path = $CONFIG['LARGE'] ;
		//600x368
		$newwidth = 600;
		$newheight = 368;
		$slider_path = $CONFIG['SLIDER'];
		//235x155
		$newwidth1 = 235;
		$newheight1 = 155;
		$medium_path = $CONFIG['MEDIUM'];
		//95x95
		$newwidth2 = 95;
		$newheight2 = 95;
		...
		foreach($files as $val){
			...
			if($val != "." && $val != ".."){
				$imageL = strtolower($image);
				$valL = strtolower($val);
				if($imageL==$valL){

					$image_info = getimagesize($path."/".$val);
					list($width,$height)=getimagesize($path."/".$val);
					...
					$hashfilename = md5($val).$type;
					$sourcefile = $path."/".$val;
					$large = $ori_path.$hashfilename;
					$slider = $slider_path.$hashfilename;
					$medium = $medium_path.$hashfilename;
					$thumb = $thumb_path.$hashfilename;
					copy($sourcefile,$large);
					...
					//600x368
					$new_image = imagecreatetruecolor(600, 368);
					imagecopyresampled($new_image, $image, 0, 0, 0, 0, 600, 368, $width, $height);
					imagejpeg($new_image,$slider,100);
					
					//235x155
					$new_image = imagecreatetruecolor(235, 155);
					imagecopyresampled($new_image, $image, 0, 0, 0, 0, 235, 155, $width, $height);
					imagejpeg($new_image,$medium,100);
					
					//95x95
					$new_image = imagecreatetruecolor(700, 700);
					imagecopy($new_image, $image, 0, 0, 324, 0, 1024, 700);
					imagejpeg($new_image,$thumb,100);

					$detail=explode('#', $val);
					$email = str_replace(".jpg","",$detail[2]);
					$date = DateTime::createFromFormat('Ymj', $detail[0]);
					$tgl = $date->format('Y-m-d');

					//check
					/*$sql="SELECT * FROM hnk_event_gallery WHERE src = '{$hashfilename}'";
					var_dump($this->apps->query($sql));exit;
					if(!$this->apps->query($sql)){*/
						$sql="INSERT INTO hnk_event_gallery (email,event,src,date_photo)
								VALUES ('{$email}','{$detail[1]}','{$hashfilename}','{$tgl}')";
						$this->apps->query($sql);
						return "Success";
					/*}else{
						return "Failed";
